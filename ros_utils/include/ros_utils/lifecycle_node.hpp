...
public:
  ...
  std::shared_ptr<util::LifecycleNode> shared_from_this()
  {
    return std::static_pointer_cast<util::LifecycleNode>(
      rclcpp_lifecycle::LifecycleNode::shared_from_this());
  }

  util::CallbackReturn on_error(const rclcpp_lifecycle::State & /*state*/)
  {
    RCLCPP_FATAL(
      get_logger(),
